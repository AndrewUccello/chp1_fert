...
##NOTE<<<---- change long might still be scaled, need to figure out before continuing 4/11
traits_change <- unique(changeLong$trait)

results_change <- lapply(traits_change, function(tr_change) {
  df_trait_change <- filter(changeLong, trait == tr_change)
  model_change <- lmer(value ~ treatment * year + (1 | block) + (1 | block:year), data = df_trait_change)
...
results_change[["deltalignin"]]$model_summary_change
results_change[["deltalignin"]]$emmeans_change
...
results_change_new <- lapply(traits_change_new, function(tr_change_new) {
  ...
  model_change_new <- lmer(value ~ treatment * year + (1 | block) + (1 | block:year), data = df_trait_change_new)
